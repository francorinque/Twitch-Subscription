...
//********* LOGICA DEL CARRITO ************//
const showModal = (msj, clase) => {
  $modalSucces.classList.add("showme");
  $modalSucces.classList.add(clase);
    ...
    return canal.id === channel.id
      ? {
          ...
          quantity:
            ...
            canal.quantity === 1 ? 3 : canal.quantity === 3 ? 6 : 1,
        }
      : canal;
  });
};
...
const addChannel = (e) => {
  ...
  if (existe(channel)) {
    ...
    addUnit(channel);
    // mostramos el modal
    showModal("Se añadio una unidad", "succes");
  } else {
    // agregamos el canal
    addChannelToCart(channel);
    // mostramos el modal
    showModal("Se añadio un canal", "succes");
  }
  checkCartState();
};
  ...
  return `
    ...
    <div class="card__info">
      <div class="card__flex">
        <a href="${url}" class="card__name" target="_blank">
          <h2>${name}</h2>
        </a>
...
const quitarUnidad = (channel) => {
  cart = cart.map((canal) => {
    return canal.id === channel.id
      ? {
          ...canal,
          quantity: canal.quantity === 6 ? 3 : canal.quantity === 3 ? 1 : 1,
        }
      : canal;
  });
};
